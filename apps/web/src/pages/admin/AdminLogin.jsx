...
const ADMIN_DEMO_EMAIL = "admin@rescuebite.test";
...
export default function AdminLogin() {
  ...
  const submit = async e => {
    ...
    if (trimmedEmail === ADMIN_DEMO_EMAIL && password === ADMIN_DEMO_PASSWORD) {
      setSession("mock-admin-token", {
        id: 1,
        name: "Admin User",
        ...
        <div className="overflow-hidden rounded-[2rem] border border-[color:var(--color-rescue-border)] bg-[color:var(--color-rescue-surface)] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.15)] lg:grid lg:grid-cols-[1.1fr_1fr]">
          <div className="relative hidden bg-[#091f1a] p-10 text-white lg:flex lg:flex-col lg:justify-between">
            ...
            <div className="relative">
              <img src="/rescuebite-logo.svg" alt="RescueBite" className="h-10 w-auto brightness-0 invert" />
              <div className="mt-28 max-w-sm">
                <p className="text-[11px] font-bold uppercase tracking-[0.25em] text-emerald-300/90">Secure access</p>
                <h1 className="mt-4 text-4xl font-black leading-[1.1] tracking-tight">RescueBite Admin Console</h1>
                <p className="mt-5 text-sm leading-7 text-white/70">Manage approvals, donations, rescues and platform activity from one place.</p>
              </div>
            </div>
          ...
          <div className="p-8 sm:p-10 lg:p-12 xl:p-14">
            <Link to="/" className="inline-flex items-center text-sm font-bold text-[#0F9F76] hover:underline underline-offset-4">← Back to RescueBite</Link>

            <div className="mt-10">
              <h2 className="text-3xl font-black tracking-tight text-[color:var(--color-rescue-text)]">Admin Sign In</h2>
              <p className="mt-2.5 text-sm text-[color:var(--color-rescue-text-muted)]">Use your admin account to continue.</p>
            </div>
            ...
            <form onSubmit={submit} className="mt-8 space-y-6">
              <label className="block">
                <span className="text-[11px] font-bold uppercase tracking-wider text-[color:var(--color-rescue-text-muted)]">Email</span>
                <input
                  required
                  type="email"
                  value={email}
              ...
              <label className="block">
                <span className="text-[11px] font-bold uppercase tracking-wider text-[color:var(--color-rescue-text-muted)]">Password</span>
            ...
            <p className="mt-6 text-center text-xs text-[color:var(--color-rescue-text-muted)]">
              Need admin access? <span className="font-semibold text-[color:var(--color-rescue-text)]">Contact administrator</span>
            </p>
          </div>
        </div>
      </div>
