import { ArrowRight, CheckCircle2, Clock3, Flag, ShieldCheck } from "lucide-react";
import { Link } from "react-router-dom";
import { adminActivities, adminNgos, adminStats, adminVolunteers } from "../../data/adminMockData";
import AdminLayout from "../../components/admin/AdminLayout";
import AdminStatCard from "../../components/admin/AdminStatCard";
import StatusBadge from "../../components/admin/StatusBadge";

export default function AdminDashboard() {
  const pendingNgos = adminNgos.filter(x => x.status === "Pending").length;
  const pendingVolunteers = adminVolunteers.filter(x => x.status === "Pending").length;
    ...
    <AdminLayout>
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-3">
        ...
        <div className="rounded-2xl border border-[color:var(--color-rescue-border)] bg-[color:var(--color-rescue-surface)] p-5 shadow-sm">
          <div className="flex items-center justify-between"><div><h2 className="font-black">System Health</h2><p className="mt-1 text-xs text-[color:var(--color-rescue-text-muted)]">Frontend demo indicators</p></div><CheckCircle2 className="text-[#0F9F76]" size={22} /></div>
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-2">
        <div className="rounded-2xl border border-[color:var(--color-rescue-border)] bg-[color:var(--color-rescue-surface)] p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            {adminActivities.slice(0, 5).map(a => <div key={a.id} className="flex gap-3 rounded-xl border border-[color:var(--color-rescue-border)] p-3"><div className="mt-0.5 h-8 w-8 shrink-0 rounded-lg bg-[color:var(--color-rescue-accent-soft)] text-center text-[#0F9F76]"><span className="relative top-1.5">•</span></div><div className="min-w-0 flex-1"><p className="text-sm font-bold">{a.title}</p><p className="mt-0.5 text-xs text-[color:var(--color-rescue-text-muted)]">{a.detail}</p></div><span className="whitespace-nowrap text-[10px] text-[color:var(--color-rescue-text-muted)]">{a.time}</span></div>)}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[color:var(--color-rescue-border)] bg-[color:var(--color-rescue-surface)] p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <Link to="/admin/reported-content" className="rounded-xl bg-red-500/5 p-4 hover:bg-red-500/10"><Flag className="text-red-500" size={19} /><p className="mt-3 text-sm font-bold">Reported content</p><p className="mt-1 text-xs text-[color:var(--color-rescue-text-muted)]">2 items awaiting review</p></Link>
